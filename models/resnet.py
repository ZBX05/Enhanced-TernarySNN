...
class SEWBasicBlock(BasicBlock):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, connect_f='ADD'):
        ...
        super(SEWBasicBlock, self).__init__(inplanes, planes, stride, downsample)
    ...
    def forward(self, s):
        ...
        if self.connect_f == 'ADD':
            ...
            raise NotImplementedError('Unknown connection function: {}'.format(self.connect_f))
...
class ResNet_Cifar(nn.Module):
    ...
    def __init__(self, block, layers, input_c=3, num_classes=10, rp = False):
        ...
        #! Solving problem in init.py: `assert len(list(net.parameters())) == len(decay) + len(no_decay)`
...
class SEWResNet(ResNet):
    def __init__(self, block, layers, input_c=3, num_classes=1000, connect_f='ADD'):
        self.connect_f = connect_f
        super(SEWResNet, self).__init__(block, layers, input_c, num_classes)
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, connect_f=self.connect_f))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes, connect_f=self.connect_f))
...
def SEWResNet34(**kwargs):
    return SEWResNet(SEWBasicBlock, [3, 4, 6, 3], **kwargs)

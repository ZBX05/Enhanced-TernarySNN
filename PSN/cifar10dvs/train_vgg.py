...
import math
import myTransform
from PSN.cifar10dvs.functions import TET_loss, seed_all, get_logger
from torch.utils.tensorboard import SummaryWriter
from PSN.cifar10dvs.spikingjelly.datasets import split_to_train_test_set
from PSN.cifar10dvs.spikingjelly.datasets.cifar10_dvs import CIFAR10DVS
from PSN.cifar10dvs.models import VGGSNN, ComplementaryTernarySpike
from PSN.cifar10dvs.datasets import TransformedSubset
...
def train(model, device, train_loader, criterion, optimizer, epoch, args):
    ...
    for i, (images, labels) in enumerate(tqdm(train_loader)):
        ...
        if args.TET:
            loss = TET_loss(outputs, labels, criterion, args.means, args.lamb, mem_list, args.lamb_mem)
        else:
...
def build_dvscifar():
    ...
    transform_test = transforms.Compose([
        ...
        transforms.Resize(size=(48, 48))])
        # transforms.Normalize(mean=[n / 255. for n in [129.3, 124.1, 112.4]], std=[n / 255. for n in [68.2, 65.4, 70.4]])
    # train_set = CIFAR10DVS(root=root_path + '/datasets/', train=True, data_type='frame', frames_number=args.T, split_by='number', transform=transform_train)
    # test_set = CIFAR10DVS(root=root_path + '/datasets/', train=False, data_type='frame', frames_number=args.T, split_by='number', transform=transform_test)
    origin_set = CIFAR10DVS(root=root_path + '/datasets/', data_type='frame', frames_number=args.T, split_by='number')
    train_set, test_set = split_to_train_test_set(0.9, origin_set, 10)
    train_set = TransformedSubset(train_set.dataset, train_set.indices, transform=transform_train)
    test_set = TransformedSubset(test_set.dataset, test_set.indices, transform=transform_test)
...
if __name__ == '__main__':
    ...
    if args.TET:
        ...
        global mem_list
        mem_list = []
        neuron_hook = lambda layer_name: lambda module, input, output: mem_list.append(module.pre_spike_mem.flatten(1, -1))
        for n, m in model.named_modules():
            if isinstance(m, ComplementaryTernarySpike):
    ...
    logger = get_logger(out_dir + f'/train.log')
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if save_max:
            torch.save(checkpoint, out_dir + '/checkpoint_max.pth')
